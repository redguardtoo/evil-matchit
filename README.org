...
* Usage
Press "%" to jump inside between tag pair in normal mode or visual mode. Please note evil-matchit is smart enough to *detect the beginning of tag automatically*.
...
(eval-after-load 'evil-matchit-ruby
  '(progn
     (add-to-list 'evilmi-ruby-extract-keywords-howtos '("^[ \t]*\\([a-z]+\\)\\( .*\\| *\\)$" 1))
...
You want a new major-mode "my-mode" to do the html tag matching? Easy. Please add below line into your ~/.emacs:
